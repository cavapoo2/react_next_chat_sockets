...
export default class NameCheck extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  validateInput(value) {
    function isValid(str) { return /^\w+$/.test(str); } //any char a-zA-Z0-9_ */
  ...
  handleChange(event) {
    //need to validate input
     ...
     this.setState({ value: event.target.value,result:true });
  }
  callApi() {
    console.log('name=',this.state.value);
    axios.get(`/name/${this.state.value}`, {
    })
      .then(res => {
  ...
  handleClear(event) {
    // this.setState({ value: '', valid: false, data: [], result: '', });
  }
  componentDidUpdate() {
    /*
          ...
          <div className="fieldset-wrapper">
            <fieldset>
              <p className="field field-text tooltip">
                <label htmlFor="comment-text">Your name:</label>
                <textarea value={this.state.value} onChange={this.handleChange} cols="10" rows="10"></textarea>
                <span className="tooltiptext">Enter your name, then hit send button</span>
              </p>
            </fieldset>
          </div>
          <p className="field-submit">
            <input disabled={!this.state.valid} value="Connect" className="submit" type="submit" />
            {!this.state.valid && this.state.value.length > 0 ? <span className="warning">Invalid code entered</span> : <span></span>}
            {!this.state.result ? <span className="warning">Name is already registered, choose another</span> : <span></span>}
          </p>
